...
class Tablero:
    ...
    def _coloca_barco(self, eslora: int):
        """
    ...
    def receive_shot(self, fila: int, col: int) -> bool:
        ...
        Returns:
            ``True`` si el disparo impacta en un barco; ``False`` si es agua.
...
#        celda = self.board[fila, col]
 ...
 #       if celda in (simbolo_hit, simbolos_miss, simbolo_sunk):
  #          raise CeldaYaDisparadaError(
   #             f"La celda ({fila}, {col}) ya recibió un disparo anteriormente."
   #Agua
#if celda == simbolo_empty:
    #self.board[fila, col] = simbolo_miss
    #return "Agua"
#Tocado
#self.board[fila, col] = simbolo_hit
